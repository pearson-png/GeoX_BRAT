...
def zonalStatsWithinBuffer(buffer, ras, statType, statField, outFC, outFCField, scratch):
    #arcpy.AddMessage((buffer, ras, statType, statField, outFC, outFCField, scratch))          
    ...
    #statTbl = arcpy.sa.ZonalStatisticsAsTable(buffer, id_field, ras,  r"C:\Users\jessie_moravek\Documents\ArcGIS\BRAT_ArcPro\Input_Layers_AllSierras\Input_Layers_AllSierras\Input_Layers_AllSierras.gdb\test", "DATA", statType, "CURRENT_SLICE", 90, "AUTO_DETECT", "ARITHMETIC", 360)
    

        
    #arcpy.AddMessage(statTbl)          
    ...
    needStatList = []
    #arcpy.AddMessage((buffer, [id_field]))
    ...
    if len(needStatList) > 0:
        ...
        #arcpy.AddMessage(quer)
        #arcpy.AddMessage(tmp_buff_lyr)
        arcpy.management.SelectLayerByAttribute(tmp_buff_lyr, 'NEW_SELECTION', quer, None)
    ...
    if len(needStatList) > 0:
        items = [stat, tmp_buff_lyr]
        for item in items:
            if item is not None:
                arcpy.Delete_management(item)
    arcpy.Delete_management(statTbl)
...
def main(
    ...
    arcpy.management.Delete(midpoint_buffer)
