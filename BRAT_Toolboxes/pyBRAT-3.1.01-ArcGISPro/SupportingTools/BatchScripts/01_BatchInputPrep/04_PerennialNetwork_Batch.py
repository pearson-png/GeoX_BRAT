...
def main():
    ...
    # make list of all directories within project folder that don't start with '00_'
    os.chdir(pf_path)
    ...
    # loop through each HUC8 folder, grab network, and run perennial network function
    for dir in dir_list:
        print("Creating perennial network for " + dir)
        # nhd_orig_flowline_path - path to original nhd flowline shapefile
        nhd_orig_flowline_path = os.path.join(pf_path, dir, 'NHD/NHDFlowline.shp')
        ...
        try:
            perennialNetwork(nhd_orig_flowline_path, nhd_area_path, nhd_waterbody_path, outpath)
