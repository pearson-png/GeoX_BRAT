...
def create_clipped_layers(output_folder, clipping_network, symbology_folder):
    ...
    inter_folders = list(filter(lambda x: os.path.isdir(os.path.join(intermediates_folder, x)),
...
def make_layer_package(output_folder, intermediates_folder, analyses_folder,
    ...
    aprx.relativePaths = False
    df = aprx.listMaps()[0]
    for lyr in df.listLayers():
        df.removeLayer(lyr)

    analyses_layer = get_analyses_layer(analyses_folder, empty_group_layer, df, aprx, clipping_network)
    inputs_layer = get_inputs_layer(empty_group_layer, inputs_folder, df, aprx, clipping_network)
    intermediates_layer = get_intermediates_layers(empty_group_layer, intermediates_folder, df, aprx, clipping_network)
    output_layer = group_layers(empty_group_layer, "Output", [intermediates_layer, analyses_layer], df, aprx)
    output_layer = group_layers(empty_group_layer, layer_package_name[:-4],
                                [output_layer, inputs_layer], df, aprx, remove_layer=False)
...
def get_analyses_layer(analyses_folder, empty_group_layer, df, aprx, clipping_network):
    """
    ...
    :param aprx: ArcMap document where layer package is being built
    ...
    existing_density_group = group_layers(empty_group_layer, "Dam Density", [existing_density_lyr], df, aprx)
    existing_complex_group = group_layers(empty_group_layer, "Size of Complex", [existing_complex_lyr], df, aprx)
    existing_capacity_layers = [existing_complex_group, existing_density_group]
    existing_capacity_layer = group_layers(empty_group_layer, "Existing Capacity", existing_capacity_layers, df, aprx)

    historic_density_group = group_layers(empty_group_layer, "Dam Density", [historic_density_lyr], df, aprx)
    historic_complex_group = group_layers(empty_group_layer, "Size of Complex", [historic_complex_lyr], df, aprx)
    historic_capacity_layers = [historic_complex_group, historic_density_group]
    historic_capacity_layer = group_layers(empty_group_layer, "Historic Capacity", historic_capacity_layers, df, aprx)

    management_layers = find_layers_in_folder(management_folder, clipping_network)
    management_layer = group_layers(empty_group_layer, "Management", management_layers, df, aprx)
    validation_layers = find_layers_in_folder(validation_folder, clipping_network)
    validation_layer = group_layers(empty_group_layer, "Validation", validation_layers, df, aprx)
    
    capacity_layer = group_layers(empty_group_layer, "Capacity",
                                  [historic_capacity_layer, existing_capacity_layer], df, aprx)
    output_layer = group_layers(empty_group_layer, "Beaver Restoration Assessment Tool - BRAT",
                                [management_layer, capacity_layer, validation_layer], df, aprx)

    return output_layer


def get_inputs_layer(empty_group_layer, inputs_folder, df, aprx, clipping_network):
    """
    ...
    :param aprx: The map document we're working with
    ...
    ex_veg_layer = group_layers(empty_group_layer, "Existing Vegetation", ex_veg_layers, df, aprx)
    hist_veg_layers = find_instance_layers(hist_veg_folder, None)
    hist_veg_layer = group_layers(empty_group_layer, "Historic Vegetation", hist_veg_layers, df, aprx)
    veg_layer = group_layers(empty_group_layer, "Vegetation", [hist_veg_layer, ex_veg_layer], df, aprx)

    network_layers = find_instance_layers(network_folder, clipping_network)
    network_layer = group_layers(empty_group_layer, "Network", network_layers, df, aprx)
    ...
    topo_layer = group_layers(empty_group_layer, "Topography",
                              hillshade_layers + dem_layers + slope_layers + flow_layers, df, aprx)

    valley_layers = find_instance_layers(valley_folder, None)
    valley_layer = group_layers(empty_group_layer, "Valley Bottom", valley_layers, df, aprx)
    road_layers = find_instance_layers(roads_folder, None)
    road_layer = group_layers(empty_group_layer, "Roads", road_layers, df, aprx)
    railroad_layers = find_instance_layers(railroads_folder, None)
    railroad_layer = group_layers(empty_group_layer, "Railroads", railroad_layers, df, aprx)
    canal_layers = find_instance_layers(canals_folder, None)
    canal_layer = group_layers(empty_group_layer, "Canals", canal_layers, df, aprx)
    land_use_layers = find_instance_layers(land_use_folder, None)
    land_use_layer = group_layers(empty_group_layer, "Land Use", land_use_layers, df, aprx)
    anthropogenic_layer = group_layers(empty_group_layer, "Anthropogenic Layers",
                                       [valley_layer, road_layer, railroad_layer, canal_layer, land_use_layer], df, aprx)

    return group_layers(empty_group_layer, "Inputs",
                        [topo_layer, veg_layer, network_layer, anthropogenic_layer], df, aprx)


def get_intermediates_layers(empty_group_layer, intermediates_folder, df, aprx, clipping_network):
    """
    ...
    # "AnthropogenicIntermediates", "Anthropogenic Intermediates", emptyGroupLayer, df, aprx)
    ...
    return group_layers(empty_group_layer, "Intermediates", intermediate_layers, df, aprx)


def find_and_group_layers(layers_list, folder_base, folder_name,
                          group_layer_name, empty_group_layer, df, aprx, clipping_network):
    """
    ...
    if folder_path:
        ...
        layers_list.append(group_layers(empty_group_layer, group_layer_name, layers, df, aprx))
...
def group_layers(group_layer, group_name, layers, df, aprx, remove_layer=True):
    """
    ...
    layers = [x for x in layers if x != None]  # remove none type from the layers

    group_layer = arcpy.mp.ArcGISProject.listMaps.listLayers(group_layer)
    group_layer.name = group_name
    group_layer.description = "Made Up Description"
    arcpy.mp.AddLayer(df, group_layer, "BOTTOM")
    group_layer = arcpy.mp.ListLayers(aprx, group_name, df)[0]

    for layer in layers:
        if isinstance(layer, arcpy.mp.Layer):
            layer_instance = layer
        else:
            layer_instance = arcpy.mp.Layer(layer)

        arcpy.mp.AddLayerToGroup(df, group_layer, layer_instance)
